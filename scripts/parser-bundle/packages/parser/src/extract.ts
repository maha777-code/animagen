...
function extractEnvironment(tokens: Token[]): { environment: EnvironmentType; matched: string[] } {
  ...
  if (hit) return { environment: hit.value, matched: [`environment:${hit.matched}`] };
...
function extractLighting(tokens: Token[]): { lighting: LightingSpec; matched: string[] } {
  ...
  if (tokens.some((t) => t.value === 'stormy' || t.value === 'storm') && !moodHit) {
    mood = 'stormy';
...
function extractMotions(tokens: Token[]) {
  ...
  const motions = hits.map((h) => ({ motion: h.value, index: h.startIndex, matched: h.matched }));
...
function extractPath(tokens: Token[]): { path?: PathType; matched: string[] } {
  ...
  if (hit) return { path: hit.value, matched: [`path:${hit.matched}`] };
...
function linkAnimations(
  ...
  for (const motion of motions) {
    ...
    animations.push({ target, motion: motion.motion, speed, ...(path ? { path } : {}), loop: true });
  }
  return animations;
}
...
function extractEffects(tokens: Token[]): { effects: EffectType[]; matched: string[] } {
  ...
  if (!seen.has('storm') && tokens.some((t) => ['stormy', 'thunder', 'tempest'].includes(t.value))) {
    effects.push('storm');
...
function extractCamera(
  ...
  for (const [phrase, movement] of Object.entries(CAMERA_PHRASES)) {
    if (matchPhrase(tokens, phrase) !== null) {
      ...
      return {
        camera: { movement, duration: 12, ...(movement === 'follow' && target ? { target } : {}) },
        matched,
      };
    }
  }
  if (motions.some((m) => m.motion === 'fly' || m.motion === 'swim')) {
    ...
    const target = subjects[0]?.name ?? subjects[0]?.type;
    return { camera: { movement: 'follow', duration: 15, ...(target ? { target } : {}) }, matched };
  }
